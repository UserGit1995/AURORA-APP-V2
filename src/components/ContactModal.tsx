...
export const ContactModal: React.FC<ContactModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-xs animate-in fade-in">
      <div 
        className="relative w-full max-w-lg bg-[#0d1420] border border-[#1c2433] rounded-3xl overflow-hidden shadow-2xl p-6 sm:p-7"
      ...
      >
        ...
        {submitted ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5">
            {/* Quick Contacts Bar */}
            <div className="grid grid-cols-2 gap-2 bg-[#0d1420] border border-[#1c2433] p-2.5 rounded-xl text-xs">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-400 mb-1">Oggetto richiesta</label>
              <input
                type="text"
                required
                value={formData.subject}
                onChange={(e) => setFormData({ ...formData, subject: e.target.value })}
                className="w-full bg-[#0d1420] border border-[#1c2433] rounded-xl px-3 py-2 text-xs text-white placeholder-slate-500 focus:outline-hidden focus:border-sky-400"
              />
            </div>

            <div>
              <label className="block text-xs font-semibold text-slate-400 mb-1">Messaggio o Note Ordine</label>
              <textarea
                required
                rows={3}
                ...
                className="w-full bg-[#0d1420] border border-[#1c2433] rounded-xl px-3 py-2 text-xs text-white placeholder-slate-500 focus:outline-hidden focus:border-sky-400 resize-none"
              />
            </div>
