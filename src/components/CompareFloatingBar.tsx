...
export const CompareFloatingBar: React.FC<CompareFloatingBarProps> = ({
  ...
  return (
    <AnimatePresence>
      ...
      >
        <div className="bg-[#0d1420]/95 backdrop-blur-md border border-[#1c2433] rounded-2xl sm:rounded-3xl p-3 sm:p-4 shadow-2xl shadow-black/80 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0 scrollbar-none">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] font-semibold bg-[#0d1420] text-sky-300 px-2 py-0.5 rounded-full border border-[#1c2433]">
            ...
            <div className="flex items-center gap-2 ml-auto sm:ml-2">
              ...
              {Array.from({ length: Math.max(0, maxCompare - comparedProducts.length) }).map((_, idx) => (
                <div
                  key={`empty-${idx}`}
                  className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl border border-dashed border-[#1c2433] bg-[#0d1420]/60 flex items-center justify-center text-slate-400 text-[10px] shrink-0"
