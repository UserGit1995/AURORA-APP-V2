...
export const CompareView: React.FC<CompareViewProps> = ({
  ...
  return (
    <div className="w-full animate-in fade-in duration-200">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          ...
          <div>
            <div className="flex items-center gap-2.5">
              ...
              {comparedProducts.length > 0 && (
                <span className="text-xs font-bold bg-slate-50 text-sky-700 px-2.5 py-1 rounded-full border border-slate-200">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button
            ...
            className="text-xs font-semibold text-sky-600 hover:text-sky-700 px-3.5 py-2 rounded-xl bg-slate-50 border border-slate-200 hover:border-sky-500/40 transition-colors"
          >
            ← Torna al catalogo
          </button>
          ...
          {comparedProducts.length > 0 && (
            <button
              ...
              className="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-400 hover:text-rose-300 px-3.5 py-2 rounded-xl bg-slate-50 border border-slate-200 hover:border-rose-500/30 transition-colors"
            >
      ...
      {comparedProducts.length === 0 ? (
        <div className="bg-slate-50 border border-slate-200 rounded-3xl p-8 sm:p-12 text-center flex flex-col items-center justify-center shadow-xl">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 mb-8">
            ...
            <button
              ...
              onClick={() => setIsAddPickerOpen(true)}
              className="inline-flex items-center gap-2 bg-slate-50 hover:bg-slate-100 text-sky-700 border border-slate-200 text-xs sm:text-sm font-semibold px-5 py-2.5 rounded-xl transition-colors"
            >
          ...
          <div className="w-full max-w-3xl pt-6 border-t border-slate-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {allProducts.slice(0, 3).map((prod) => (
                <div
                  key={prod.id}
                  className="bg-slate-50 border border-slate-200 rounded-2xl p-3 flex items-center justify-between gap-3 text-left"
                >
                  <img
                    src={prod.image}
                    alt={prod.name}
                  ...
                  <button
                    ...
                    className="p-1.5 rounded-lg bg-slate-50 hover:bg-[#0284c7] text-sky-600 hover:text-white transition-colors"
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-2xl p-3 sm:p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-md">
            <div className="flex items-center gap-3">
              <label 
              ...
              >
                <div className="relative">
                  <input
                    ...
                    className="sr-only"
                  />
                  <div className={`w-9 h-5 rounded-full transition-colors ${highlightDifferences ? 'bg-[#0284c7]' : 'bg-slate-50'}`} />
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-3xl overflow-hidden shadow-2xl">
            <div className="overflow-x-auto scrollbar-thin">
              <table className="w-full text-left border-collapse min-w-[640px]">
                {/* 1. Header Row: Product Summaries & Action Cards */}
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50">
                    <th className="p-4 sm:p-5 w-48 sm:w-60 text-xs font-bold text-slate-400 uppercase tracking-wider align-top bg-slate-50 border-r border-slate-200">
                    ...
                    {comparedProducts.map((product) => {
                      ...
                      return (
                        <th
                          key={product.id}
                          className="p-4 sm:p-5 min-w-[220px] max-w-[280px] align-top relative group border-r last:border-r-0 border-slate-200"
                        >
                          {/* Remove button */}
                          <button
                            id={`remove-compared-col-${product.id}`}
                            onClick={() => onRemoveFromCompare(product.id)}
                            className="absolute top-3 right-3 p-1.5 rounded-full bg-slate-50 hover:bg-rose-500/20 text-slate-400 hover:text-rose-300 border border-slate-200 transition-colors"
                    ...
                    {comparedProducts.length < 4 && (
                      <th className="p-4 sm:p-5 min-w-[200px] align-middle text-center bg-slate-50/50">
                        <button
                          id="add-more-compare-slot-btn"
                          onClick={() => setIsAddPickerOpen(true)}
                          className="w-full h-full min-h-[220px] rounded-2xl border-2 border-dashed border-slate-200 hover:border-sky-500/50 bg-slate-50/50 hover:bg-slate-100 flex flex-col items-center justify-center p-4 transition-all group"
                        >
                          <div className="w-10 h-10 rounded-full bg-slate-50 text-sky-600 flex items-center justify-center group-hover:scale-110 transition-transform mb-2">
                ...
                <tbody className="divide-y divide-[#102340] text-xs">
                  ...
                  <tr className="bg-slate-50/80">
                    <td 
                      ...
                      className="px-4 py-2.5 font-bold text-sky-700 uppercase tracking-wider text-[11px]"
                    >
                  ...
                  {(() => {
                    ...
                    return (
                      <tr className={`hover:bg-slate-100/50 ${diff && highlightDifferences ? 'bg-amber-500/10' : ''}`}>
                        <td className="p-3.5 sm:p-4 font-semibold text-slate-400 bg-slate-50 border-r border-slate-200">
                          Prezzo Unitario Netto
                        </td>
                        {comparedProducts.map((p) => (
                          <td key={p.id} className="p-3.5 sm:p-4 font-mono font-bold text-slate-900 border-r last:border-r-0 border-slate-200">
                            €{p.price.toFixed(2)}
                            {diff && (
                        ...
                        {comparedProducts.length < 4 && <td className="bg-slate-50/50" />}
                      </tr>
                    );
                  })()}
                  ...
                  {(() => {
                    ...
                    return (
                      <tr className={`hover:bg-slate-100/50 ${diff && highlightDifferences ? 'bg-amber-500/10' : ''}`}>
                        ...
                        {comparedProducts.map((p) => (
                          <td key={p.id} className="p-3.5 sm:p-4 font-mono text-slate-600 border-r last:border-r-0 border-slate-200">
                  ...
                  {(() => {
                    ...
                    return (
                      <tr className={`hover:bg-slate-100/50 ${diff && highlightDifferences ? 'bg-amber-500/10' : ''}`}>
                        ...
                        {comparedProducts.map((p) => (
                          <td key={p.id} className="p-3.5 sm:p-4 border-r last:border-r-0 border-slate-200">
                            {p.discountPercent ? (
                  ...
                  {(() => {
                    ...
                    return (
                      <tr className={`hover:bg-slate-100/50 ${diff && highlightDifferences ? 'bg-amber-500/10' : ''}`}>
                        ...
                        {comparedProducts.map((p) => (
                          <td key={p.id} className="p-3.5 sm:p-4 font-medium text-slate-900 border-r last:border-r-0 border-slate-200">
                            <div className="flex items-center gap-1.5">
                  ...
                  {(() => {
                    ...
                    return (
                      <tr className={`hover:bg-slate-100/50 ${diff && highlightDifferences ? 'bg-amber-500/10' : ''}`}>
                        ...
                        {comparedProducts.map((p) => (
                          <td key={p.id} className="p-3.5 sm:p-4 text-slate-700 border-r last:border-r-0 border-slate-200">
                            {p.specs.format}
                          </td>
                        ))}
                        {comparedProducts.length < 4 && <td className="bg-slate-50/50" />}
                  ...
                  {(() => {
                    ...
                    return (
                      <tr className={`hover:bg-slate-100/50 ${diff && highlightDifferences ? 'bg-amber-500/10' : ''}`}>
                        ...
                        {comparedProducts.map((p) => (
                          <td key={p.id} className="p-3.5 sm:p-4 text-slate-600 border-r last:border-r-0 border-slate-200">
                            {p.specs.fragrance ? (
                            ...
                            {p.specs.certifications && p.specs.certifications.length > 0 ? (
                              <div className="flex flex-wrap gap-1.5">
                                {p.specs.certifications.map((cert, idx) => (
                                  <span
                                    key={idx}
                                    className="inline-flex items-center gap-1 bg-slate-50 text-sky-700 border border-slate-200 px-2 py-0.5 rounded-md text-[10.5px] font-medium"
                                  >
                  ...
                  {(() => {
                    return (
                      <tr className="hover:bg-slate-100/50">
                        ...
                        {comparedProducts.map((p) => (
                          <td key={p.id} className="p-3.5 sm:p-4 text-slate-600 leading-relaxed border-r last:border-r-0 border-slate-200 text-xs">
                            {p.description}
                          </td>
                        ))}
                        {comparedProducts.length < 4 && <td className="bg-slate-50/50" />}
                    ...
                    {comparedProducts.map((p) => (
                      <td key={p.id} className="p-3.5 sm:p-4 border-r last:border-r-0 border-slate-200">
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 text-xs text-sky-600 hover:text-sky-700 hover:underline font-medium"
                        >
        ...
        {isAddPickerOpen && (
          <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-2xl bg-slate-50 border border-slate-200 rounded-3xl p-5 sm:p-6 shadow-2xl z-10 max-h-[85vh] flex flex-col"
            >
              <div className="flex items-center justify-between pb-4 border-b border-slate-200 mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-slate-900">Aggiungi al Confronto</h3>
              ...
              <div className="overflow-y-auto space-y-2 pr-1 flex-1">
                {availableToAdd.length === 0 ? (
                  ...
                  availableToAdd.map((product) => (
                    <div
                      key={product.id}
                      className="bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-2xl p-3 flex items-center justify-between gap-3 transition-colors"
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        <img
                          ...
                          alt={product.name}
                          referrerPolicy="no-referrer"
                          className="w-12 h-12 object-contain rounded-xl bg-slate-50 p-1 shrink-0"
                        />
              ...
              <div className="mt-4 pt-3 border-t border-slate-200 flex justify-end">
                <button
                  onClick={() => setIsAddPickerOpen(false)}
                  className="px-4 py-2 rounded-xl text-xs font-semibold bg-slate-50 text-slate-600 hover:text-slate-900"
                >
                  Chiudi
                </button>
              </div>
